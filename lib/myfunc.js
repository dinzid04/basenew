...
exports.smsg = async (conn, m, store) => {
  ...
  if (m.key) {
    ...
    if (rawSender.includes(':')) rawSender = jidNormalizedUser(rawSender);
    m.sender = rawSender;

    // LOGIKA KHUSUS LID FIX (Diambil dari myfuncyoi & LynxxxBackup_Botz)
    if (m.sender && m.sender.endsWith('@lid')) {
        let fixedSender = m.sender;

        // 1. Cek di database lokal dulu
        if (lidDatabase[m.sender] && lidDatabase[m.sender].jid) {
            fixedSender = lidDatabase[m.sender].jid;
        }
        // 2. Cek di Store contacts
        else if (store && store.contacts) {
            const contact = Object.values(store.contacts).find(c => c.lid === m.sender);
            if (contact && contact.id && !contact.id.endsWith('@lid')) {
                fixedSender = contact.id;
                // Simpan ke database biar besok2 gak nyari lagi
                lidDatabase[m.sender] = { jid: contact.id };
                try { fs.writeFileSync(lidPath, JSON.stringify(lidDatabase, null, 2)); } catch {}
            }
        }

        // 3. Fallback ke Metadata (Jika Group)
        if (fixedSender.endsWith('@lid') && m.isGroup) {
             const groupMetadata = await conn.groupMetadata(m.chat).catch(_ => null);
             if (groupMetadata && groupMetadata.participants) {
                 const found = groupMetadata.participants.find(p => p.lid === m.sender || p.id === m.sender);
                 if (found) {
                     const realJid = found.jid || found.id;
                     if (realJid && !realJid.endsWith('@lid')) {
                         fixedSender = realJid;
                         lidDatabase[m.sender] = { jid: realJid };
                         try { fs.writeFileSync(lidPath, JSON.stringify(lidDatabase, null, 2)); } catch {}
                     }
                 }
             }
  ...
  if (m.message) {
        ...
        m.mentionedJid = m.mentionedJid.map(jid => {
            if (jid.endsWith('@lid')) {
                ...
                if (store && store.contacts) {
                    ...
                    if (contact && contact.id && !contact.id.endsWith('@lid')) {
                        lidDatabase[jid] = { jid: contact.id }; // Save for next time
                        try { fs.writeFileSync(lidPath, JSON.stringify(lidDatabase, null, 2)); } catch {}
                        return contact.id;
                    }
                }
                // Last resort: If we can't find it, we can't really guess.
                // However, we can try to check if we are in a group and can find it in metadata if we had it.
                // But typically mentions are known.
            }
            return jid;
        });
    }
    ...
    if (m.quoted) {
      ...
      if (m.quoted.sender && m.quoted.sender.endsWith('@lid')) {
           let fixedQuotedSender = m.quoted.sender;
           if (lidDatabase[m.quoted.sender] && lidDatabase[m.quoted.sender].jid) {
               fixedQuotedSender = lidDatabase[m.quoted.sender].jid;
           } else if (store && store.contacts) {
               const contact = Object.values(store.contacts).find(c => c.lid === m.quoted.sender);
               if (contact && contact.id && !contact.id.endsWith('@lid')) {
                   fixedQuotedSender = contact.id;
                   lidDatabase[m.quoted.sender] = { jid: contact.id };
                   try { fs.writeFileSync(lidPath, JSON.stringify(lidDatabase, null, 2)); } catch {}
               }
           }
           // Fallback with metadata if possible (harder here without group context explicitly passed sometimes)

           m.quoted.sender = fixedQuotedSender;
      }
